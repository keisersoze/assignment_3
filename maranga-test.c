...
int main() {
	matrix<long, 400, 5000> mA;
	initializeCells(mA, 12, 5);
	matrix<long, 400, 5000> mB;
	initializeCells(mB, 7, 13);

	matrix<long, 5000, 2000> mC;
	initializeCells(mC, 3, 8);
	matrix<long, 5000, 2000> mD;
	initializeCells(mD, 4, 7);

	matrix<long, 2000, 400> mE;
	initializeCells(mE, 8, 9);
	matrix<long, 2000, 400> mF;
	initializeCells(mF, 5, 11);

	matrix<long, 400, 1001> mG;
	initializeCells(mG, 54, 11);
	matrix<long, 1001, 400> mH;
	...
	long first = multiplication.get_future().get()(0, 0);
